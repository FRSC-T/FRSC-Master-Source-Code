...
uint256 hashGenesisBlock("0xf3e1db75207ab327e14e0de9b2f973572838766ff484d5999e1c32636e1fb4f2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // federalreserveCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "federalreserveCoin Signed Message:\n";
...
{
    // federalreserveCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight < 43201)
    {  nSubsidy = 268000 * COIN;}
    else if (nHeight < 86401)
    { nSubsidy =214400 * COIN;}
    else if (nHeight < 129601)
    { nSubsidy = 171520 * COIN;}
    else if (nHeight < 172801)
    { nSubsidy = 137216 * COIN;}
    else if (nHeight < 216001)
    {nSubsidy =109772.8 * COIN;}
    else if (nHeight < 259201)
    {nSubsidy =87818.24 * COIN;}
    else if (nHeight < 302401)
    {nSubsidy =70254.59 * COIN;}
    else if (nHeight < 345601)
    {nSubsidy =56203.67 * COIN;}
    else if (nHeight < 388801)
    {nSubsidy =44962.94 * COIN;}
    else if (nHeight < 432001)
    {nSubsidy =35970.35 * COIN;}
    else if (nHeight < 475201)
    {  nSubsidy = 28776.28 * COIN;}
    else if (nHeight < 518401)
    { nSubsidy =23021.02 * COIN;}
    else if (nHeight < 561601)
    { nSubsidy = 18416.82 * COIN;}
    ...
    {nSubsidy =18416.82 * COIN;}
    else if (nHeight < 734401)
    {nSubsidy =18416.82 * COIN;}
    else if (nHeight < 777601)
    {nSubsidy =18416.82 * COIN;}
    else if (nHeight < 820801)
    {nSubsidy =18416.82 * COIN;}
    else if (nHeight < 864001)
    {nSubsidy =18416.82 * COIN;}
    else if (nHeight < 907201)
    {  nSubsidy = 18416.82 * COIN;}
    else if (nHeight < 950401)
    { nSubsidy =18416.82 * COIN;}
    else if (nHeight < 993601)
    { nSubsidy = 9208.41 * COIN;}
    else if (nHeight < 1036801)
    { nSubsidy = 4604.20 * COIN;}
    else if (nHeight < 1555201)
    {nSubsidy =2302.10 * COIN;}
    else if (nHeight < 2073601)
    {nSubsidy =1151.05 * COIN;}
    else if (nHeight < 2592001)
    {nSubsidy =575.53 * COIN;}
    else if (nHeight < 3110401)
    {nSubsidy =287.76 * COIN;}
    else if (nHeight < 3628801)
    {nSubsidy =143.88 * COIN;}
    else if (nHeight < 4147201)
    {nSubsidy =2 * COIN;}
    else if (nHeight < 4665601)
    {  nSubsidy = 71.94 * COIN;}
    else if (nHeight < 5184001)
    { nSubsidy =35.97 * COIN;}
    else if (nHeight < 5702401)
    { nSubsidy = 17.99 * COIN;}
    else if (nHeight < 6220801)
    { nSubsidy = 8.99 * COIN;}
    else if (nHeight < 6739201)
    {nSubsidy =4.50 * COIN;}
    else if (nHeight < 7257601)
    {nSubsidy =2.25 * COIN;}
    else if (nHeight < 7776001)
    {nSubsidy =1.12 * COIN;}
    else if (nHeight < 8294401)
    {nSubsidy =0.56 * COIN;}
    else if (nHeight < 8812801)
    {nSubsidy =0.28 * COIN;}
    else if (nHeight < 9331201)
    {nSubsidy =0.14 * COIN;}
    else if (nHeight < 9849601)
    {  nSubsidy = 0.07 * COIN;}
    else if (nHeight < 10368001)
    { nSubsidy =0.04 * COIN;}
    else if (nHeight < 10886401)
    { nSubsidy = 0.02 * COIN;}
   ...
   // nSubsidy >>= (nHeight / 840000); // federalreserveCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 20 * 60; // federalreserveCoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // federalreserveCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // federalreserveCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // federalreserveCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // federalreserveCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf3e1db75207ab327e14e0de9b2f973572838766ff484d5999e1c32636e1fb4f2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Today,The Federalreserve coins is building to world for now....";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1388211408;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085725235;
        ...
        assert(block.hashMerkleRoot == uint256("0x1dc15a2e3b1da16855a4e4d77233e9975e1fc3df7426b00d034a5404d5c41638"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // federalreserveCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("federalreserveCoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("federalreserveCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("federalreserveCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static federalreserveCoinMiner(CWallet *pwallet)
{
    printf("federalreserveCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("federalreserveCoin-miner");
    ...
    try { loop {
        ...
        printf("Running federalreserveCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("federalreserveCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&federalreserveCoinMiner, pwallet));
}
